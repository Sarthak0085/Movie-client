...
const Chat = ({ movieId, user }:{movieId: string, user: User}) => {
    const [messages, setMessages] = useState<Message[]>([]);
    const [message, setMessage] = useState('');
    const [socket, setSocket] = useState<Socket<DefaultEventsMap, DefaultEventsMap>| null>(null);
    const chatContainerRef:  MutableRefObject<HTMLDivElement | undefined>= useRef<HTMLDivElement>(undefined!);
    ...
    // Ensure the chat container reference exists
    ...
    return (
        <div className="lg:w-[30%] md:[15%] mt-10 lg:mt-0 flex flex-col w-full py-20">
            <div ref={chatContainerRef as React.LegacyRef<HTMLDivElement>} className="bg-dry-300 lg:h-[550px] space-y-3 border p-3 border-border overflow-y-auto text-white">
                {
